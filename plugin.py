...
from utilities import AchvEnum, AchvExtra, GroupMemberOp, GroupOp, Msg, MsgOp, Overrides, ProxyContext, Redirected, ResolverMixer, Source, SourceOp, Target, User, bind, ensure_attr, get_logger, to_unbind
        ...
        async def wrapper(self: 'Plugin', *args, **kwargs):
            async def ctx_wrapper(ctx: 'Context'):
                ...
                async def task():
                    try:
                        ...
                        if InstrAttr.FORCE_BACKUP in attr:
            ...
            if ctx is None:
                with self.engine.of() as c, c:
                    return await with_custom(c)
            else:
                return await with_custom(ctx)
...
class Context(ResolverMixer):
            ...
            for plugin in plugins:
                ...
                for _, method in inspect.getmembers(plugin, predicate=inspect.ismethod):
                    if hasattr(method, instr_attr_name):
                        ...
                        if InstrAttr.FORCE_BACKUP in method._instr_attrs_:
    ...
    async def send(self):
        ...
        if InstrAttr.NO_ALERT_CALLER not in [e for e in attrs]:
            if self.redirected.to is None:
                if self.redirected.source_op.source.get_target() == Target.GROUP:
                    append_at(self.redirected.source_op.source.get_member_id())
            else:
    ...
    def resolver_mixin(self) -> Dict[type, Callable[..., Any]]:
        ...
        def resolve_source(event: Event, group: Optional[Group], member: Optional[GroupMember]):
            return Source(event=event, group=group, member=member)
        def resolve_source_op(event: Event, source: Source):
            return SourceOp(bot=self.engine.bot, source=source)
        
        async def resolve_proxy_context():
            override = await self.get_override(ProxyContext)
            if override is None:
                raise ExecFailedError(f'本指令只能在代理上下文中执行')
            # print(f'{override=}')
            return override
        ...
        return {
            ...
            Quote: resolve_quote,
            ProxyContext: resolve_proxy_context
        }
            ...
            async def append_single_arg():
                ...
                try:
                    ...
                    if issubclass(anno, Enum):
                        ...
                        return
                    if isinstance(curr_arg, anno): # TODO
                        args.append(curr_arg)
                        return
...
def timer(interval_s_or_cb: float=1, *, exactly: bool=True):
    interval_s = 1
    def deco(fn):
        @autorun
        async def wrapper(self: 'Plugin', ctx: Context):
            await asyncio.sleep(5)
            real_delay = interval_s
            while True:
                ...
                try:
                    ...
                    ...
        return wrapper
    if callable(interval_s_or_cb):
        return deco(interval_s_or_cb)
    else:
        interval_s = interval_s_or_cb
    return deco
...
class AchvCustomizer():
    ...
    async def get_process(self, e: 'AchvEnum') -> int:
        ...
