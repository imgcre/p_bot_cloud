...
class Throttle(Plugin):
    ...
    MIN_COOLDOWN_DURATION: Final = 3 * 60

    @top_instr('cd')
    @throttle_config(name='查询冷却', max_cooldown_duration=5 * 60)
    async def get_cd_cmd(self, member: GroupMember):
        async with self as passed:
            if not passed: return
            
            res = []
            for p in self.engine.plugins.values():
                for _, method in inspect.getmembers(p, predicate=inspect.ismethod):
                    fn = to_unbind(method)
                    if not guard_attr(fn, throttle_config):
                        continue
                    ...
                    for member_id in associated:
                        ...
                        async with self.override(curr_member):
                            cooldown_reamins = max(cooldown_reamins, await self.get_cooldown_reamins(fn=fn))

                    if cooldown_reamins <= 0:
                        continue
                    
                    config = ensure_attr(fn, throttle_config)
                    res.append(f'{config.name}: {get_delta_time_str(cooldown_reamins, use_seconds=False)}')
    ...
    async def get_cooldown_reamins(self, man: ThrottleMan, *, fn: Callable=None):
        ...
        if config.effective_speedup:
            effective_speech_cnt = await self.achv.get_achv_process(MeowAchv.CACTUS)
    ...
    @delegate()
    async def do(self, man: ThrottleMan, member: GroupMember, msg_op: Optional[MsgOp], source: Source,
        *, recall: bool=True, cooldown_reamins: Optional[float]=None, fn: Callable=None
    ):
        if fn is None:
            fn = self._get_caller_fn()
        ...
        conf = ensure_attr(fn, throttle_config)

        if conf.allow_super and member.id in config.SUPER_ADMINS:
            return True
        ...
        try:
            if not conf.silent:
                if (
                    ...
                    reason = f'使用{conf.name}功能过于频繁'
                    await self.admin.inc_violation_cnt(reason=reason, hint=reason)
                if msg_op is not None and recall:
                    text = [
                        f'{conf.name}功能冷却中, 请{get_delta_time_str(cooldown_reamins, use_seconds=False)}后再试'
                    ]
                    if conf.effective_speedup:
                        text.append('多多发言可以大幅减少冷却时间哦')

                    await source.op.send([At(target=source.get_member_id()), ' ', ', '.join(text)])
                    # await self.nap_cat.send_msg(text=', '.join(text))
    ...
    async def reset(self, man: ThrottleMan, *, fn: Callable=None):
        ...
        man.fn_infos[fn.__qualname__] = FnThrottleInfo(
            effective_speech_cnt_snapshot=await self.achv.get_achv_process(MeowAchv.CACTUS),
        )
