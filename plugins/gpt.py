...
class Gpt(Plugin):
    ...
    async def on_violation(self, event: ViolationEvent, group: Group):
        ...
        typ = '违规' if not event.dec else '遵守规则'
        inc_or_dec = '降低' if not event.dec else '增加'

        if event.hint is None:
            msg = f'"{who}"{typ}了, 违规次数(猫德)计数{inc_or_dec}至{-event.count}'
        else:
            msg = f'"{who}"{typ}了, 原因: {event.hint}, 违规次数(猫德)计数{inc_or_dec}至{-event.count}'
    ...
    async def enhanced_run(self, *, comps: Optional[list[MessageComponent]]=None, recall: bool=False):
        if comps is not None:
            ...
            # if txt_len < 10 or txt_len > 50 or will_exit:
