...
class CollectedAchvMan():
    ...
    def has_static(self, e: AchvEnum):
    ...
    def get_achv_extra(self, e: AchvEnum):
        if not self.has_static(e): return None
        return self.achvs[e]
...
class Achv(Plugin, InjectNotifier):
    ...
    @delegate()
    async def remove(self, e: AchvEnum, man: Optional[CollectedAchvMan], *, force: bool=False, notify: bool=True):
        if man is None:
            return None
        ...
        if e in man.achvs:
            ...
            if info.opts.custom_remove:
                p: AchvCustomizer = next((k for k, v in self.registed_achv.items() if v is e.__class__))
                await p.remove_achv(e, man.achvs[e])
                # logger.info(f'{man.achvs[e].obtained_cnt=}')
            else:
                man.achvs.pop(e)

            if notify:
                await self.events.emit(AchvRemovedEvent(e))
    ...
    @delegate(InstrAttr.FORCE_BACKUP)
    async def submit(
        self, e: AchvEnum, member: GroupMember, op: GroupOp, man: CollectedAchvMan, 
        *, override_obtain_cnt: int = None, silent: bool = False, by: Callable[['AchvExtra'], Awaitable[None]] = None
    ):
        info = typing.cast(AchvInfo, e.value)
        prev_obtained_cnt = 0
    ...
    @delegate()
    async def get_obtained(self, man: Optional[CollectedAchvMan], *, include_hidden: bool=False) -> set[Enum]:
        if man is None: return []
        obtained_achvs = set([achv_enum for achv_enum in man.achvs.keys() if man.has_static(achv_enum) and (not (achv := typing.cast(AchvInfo, achv_enum.value)).opts.hidden or include_hidden)])
        for p, em in self.registed_achv.items():
            ...
            for e in em:
                ...
                if await p.is_achv_obtained(e):
                    obtained_achvs.add(e)
                else:
                    if e in obtained_achvs:
                        obtained_achvs.remove(e)
        return obtained_achvs
    
    @delegate()
    async def get_processing(self, man: Optional[CollectedAchvMan], *, include_hidden: bool=False):
        if man is None: return []
        achv_process_infos = set(man.achvs.keys())
        for p, em in self.registed_achv.items():
            if not isinstance(p, AchvCustomizer): continue
            for e in em:
                e: AchvEnum
                ...
                if not info.opts.dynamic_obtained: continue
                if await p.is_achv_obtained(e):
                    achv_process_infos.add(e)
                else:
                    if e in achv_process_infos:
                        achv_process_infos.remove(e)
        return achv_process_infos
        ...
    ...
    async def has(self, e: AchvEnum, man: Optional[CollectedAchvMan]):
        ...
        info: AchvInfo = e.value
        if info.opts.dynamic_obtained:
            p: AchvCustomizer = next((k for k, v in self.registed_achv.items() if v is e.__class__))
            return await p.is_achv_obtained(e)
        else:
            return man.has_static(e)
    
    @delegate()
    async def get_obtained_member_ids(self, e: AchvEnum, group: Group):
        result = set()
        for member_id, _ in self.gls.get_data_of_group(group.id).items():
            member = await self.bot.get_group_member(group.id, member_id)
    ...
    @delegate()
    async def get_achv_process(self, e: AchvEnum, man: Optional[CollectedAchvMan]):
        if e.opts.dynamic_process:
            ...
        else:
            extra = man.achvs.get(e, None)
            if extra is None: 
                return
    ...
    async def get_used(self, man: Optional[CollectedAchvMan]):
        ...
        if man._using is None:
            return None
        
        if not await self.has(man._using):
            return None
    ...
    async def is_used(self, e: AchvEnum, man: Optional[CollectedAchvMan]):
        return await self.get_used() is e
    
    @delegate()
    async def set_used(self, e: AchvEnum, man: Optional[CollectedAchvMan]):
        if man._using is not None:
            info: AchvInfo = man._using.value
            if info.opts.min_display_durtion is not None and time.time() - man.last_using_ts <= info.opts.min_display_durtion:
                raise RuntimeError(f'目前还不能卸下称号"{info.get_display_text()}"')
                ...
        man.last_using_ts = time.time()
        man._using = e
    ...
    async def award(self, at: At, aka: str):
        async with self.admin.privilege(type=AdminType.SUPER):
            ...
            async with self.override(member):
                await self.submit(e, silent=True)
            return 'ok'
    ...
    @delegate()
    async def get_achv_name(self, e: AchvEnum, man: Optional[CollectedAchvMan]):
        if not e.opts.dynamic_name:
            return e.aka
        
        extra: Optional['AchvExtra'] = None
        if man is not None and e in man.achvs:
            extra = man.achvs[e]

        p: AchvCustomizer = next((k for k, v in self.registed_achv.items() if v is e.__class__))
        return await p.get_achv_name(e, extra)
    ...
    async def disp_achv(self, man: Optional[CollectedAchvMan], member: GroupMember):
    ...
    async def use_achv(self, aka: str, man: Optional[CollectedAchvMan]):
        await self.admin.check_proxy(disable_required=True)
        for meta in self.registed_achv.values():
            e: AchvEnum = next((e for e in meta if (val := typing.cast(AchvInfo, e.value)).aka == aka), None)
            if e is not None:
                break
        else:
            return f'不存在名叫"{aka}"的成就'
        
        if not await self.has(e):
            return f'尚未获得成就"{aka}"'
        
        self.backup_man.set_dirty()
        await self.set_used(e)
    ...
    async def drop_achv(self, man: Optional[CollectedAchvMan]):
        ...
        used_achv = await self.get_used()
        if used_achv is None:
            return '当前没有佩戴任何成就'
        
        self.backup_man.set_dirty()
        await self.set_used(None)
    ...
    async def update_member_name(self, member: GroupMember, man: Optional[CollectedAchvMan]):
        ...
        max_display_priorities: dict[int, int] = {}
        if man is not None:
            def add_to_weared(info: AchvInfo):
                nonlocal max_display_priorities
                weared_achv_info_kv[info.get_display_text()] = info
                if info.opts.display_priority is not None and (
                    info.opts.display_priority_group not in max_display_priorities 
                    or info.opts.display_priority > max_display_priorities[info.opts.display_priority_group]
                ):
                    max_display_priorities[info.opts.display_priority_group] = info.opts.display_priority
            ...
            for priority_group, max_display_priority in max_display_priorities.items():
                weared_achv_info_kv = {
                    k: info for k, info in weared_achv_info_kv.items() 
                    if info.opts.display_priority is None or info.opts.display_priority_group != priority_group or info.opts.display_priority >= max_display_priority
                }
