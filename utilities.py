...
def voucher_round_half_up(cnt: Decimal):
    if not isinstance(cnt, Decimal):
        cnt = Decimal(cnt)
    
    unit = Decimal('0.01')
    return cnt.quantize(unit, ROUND_HALF_UP).normalize()
...
@dataclass
class AchvExtra(Upgraded):
    create_ts: float = field(default_factory=time.time)
    obtained_cnt: int = field(default=0)
...
class AchvOpts():
    ...
    dynamic_process: bool = field(compare=False, default=False)
    custom_remove: bool = field(compare=False, default=False)
    dynamic_name: bool = field(compare=False, default=False)
    display_priority: Optional[int] = field(compare=False, default=None)
    display_priority_group: int = field(compare=False, default=0)
    custom_progress_str: bool = field(compare=False, default=False)
...
class RewardOpts():
    ...
    use_inventory_check: bool = field(compare=False, default=False)
    desc: Optional[str] = field(compare=False, default=None)
...
class Source():
    ...
    def op(self):
        ...
        return SourceOp(Engine.get_context().engine.bot, self)
    ...
    def get_message_id(self):
        if isinstance(self.event, GroupMessage):
            return self.event.message_chain.message_id
...
class SourceOp():
    ...
    async def send(self, msg, *, to: Target=None):
        if to is None:
            to = self.source.get_target()

        if to == Target.GROUP:
            try:
                return await self.bot.send_group_message(self.source.get_group_id(), msg)
            except:
                print(f'ERR {msg=}')
                raise
        ...
        if to == Target.TEMP:
            async def map_at(c):
                if isinstance(c, At):
                    if c.target == self.source.get_member_id():
                        return '你'
                    at_member = await self.bot.get_group_member(self.source.get_group_id(), c.target)
                    return f'@{at_member.get_name()}({c.target})'
                return c
            
            at_mapped = [await map_at(c) for c in msg]
            return await self.bot.send_temp_message(self.source.get_member_id(), self.source.get_group_id(), at_mapped)
...
def deserialize(type: Type[T], data: Data) -> T:
    if hasattr(type, '__T__'):
        type = type.__T__
    if hasattr(type, '__supertype__'):
        type = type.__supertype__
    return _build_value(type_=type, data=data, config=Config())

@dataclass(unsafe_hash=True)
class RecallItem():
    msg_id: int
    target_id: int
    recall_ts: Optional[float] = field(default=None, compare=False)

    @staticmethod
    def dummy():
        return RecallItem(0, 0)

    def recall_after(self, t_s: float):
        self.recall_ts = time.time() + t_s

    def recall(self):
        self.recall_after(0)

@dataclass
class VoucherRecordExtraClearMute():
    exchange_duration: float

@dataclass
class VoucherRecordExtraAi():
    ...
...
@dataclass
class VoucherRecordExtraMora():
    player_win: bool

@dataclass
class VoucherRecordExtraReward():
    reward: RewardEnum
    ...

@dataclass
class VoucherRecordExtraAllIn():
    player_win: bool

@dataclass
class VoucherRecordExtraLiveAnswerExplanation():
    ...

@dataclass
class VoucherRecordExtraLiveGift():
    ...

@dataclass
class VoucherRecordExtraLiveCdkey():
    ...

@dataclass
class VoucherRecordExtraLiveGuard():
    ...

@dataclass
class VoucherRecordExtraReCheckIn():
    ...

@dataclass
class VoucherRecordExtraLiveGiftCompensation():
    ...
